...
def swq_search(items: list, elem) -> int:  # item是参数，list是items的注释  int是函数返回值的注释
    # 顺序查找
...
def main():
    ...
    for index, y_data in enumerate(y_datas):
        pyplot.plot(x_data, y_data, styles[index])  # 做图
    pyplot.legend(legends)  # 图示
    pyplot.xticks(numpy.arange(1, 7, step=1))  # 坐标轴
